...
CHIP_ERROR AppTask::Init()
{
    ...
    if (err != CHIP_NO_ERROR)
    {
        K32W_LOG("InitBindingHandler() failed");
        assert(err == CHIP_NO_ERROR);
    }
    return err;
}
...
void AppTask::ActionInitiated(LightingManager::Action_t aAction, int32_t aActor)
{
    ...
    {
        K32W_LOG("Switch turning on")
    }
    else if (aAction == LightingManager::TURNOFF_ACTION)
    {
        K32W_LOG("Switch turning off")
    }
...
void AppTask::ActionCompleted(LightingManager::Action_t aAction)
{
    ...
    if (aAction == LightingManager::TURNON_ACTION)
    {
        K32W_LOG("Switch is in on state")
        sLightLED.Set(true);
    }
    else if (aAction == LightingManager::TURNOFF_ACTION)
    {
        K32W_LOG("Switch is in off state")
        sLightLED.Set(false);
    }
...
void AppTask::UpdateClusterStateInternal(intptr_t arg)
{
    ...
    EmberAfStatus status = emberAfWriteAttribute(1, ZCL_SWITCH_CLUSTER_ID, ZCL_SWITCH_CLUSTER_ID, CLUSTER_MASK_SERVER,
                                                (uint8_t *) &newValue, ZCL_BOOLEAN_ATTRIBUTE_TYPE);
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        ChipLogError(NotSpecified, "ERR: updating switch state %x", status);
    }
}
...
void AppTask::UpdateDeviceStateInternal(intptr_t arg)
{
	...
	(void)emberAfReadAttribute(1, ZCL_SWITCH_CLUSTER_ID, ZCL_SWITCH_ACTIONS_ATTRIBUTE_ID, CLUSTER_MASK_SERVER,
