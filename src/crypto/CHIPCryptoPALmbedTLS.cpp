...
#if defined(MBEDTLS_USE_TINYCRYPT)
    memcpy(mbedtls_pk_uecc(pk), to_const_keypair(&mKeypair), sizeof(mbedtls_uecc_keypair));
#else
    memcpy(mbedtls_pk_ec(pk), to_const_keypair(&mKeypair), sizeof(mbedtls_ecp_keypair));
#endif
